...
#define MAX_PSRAM     (3 * 1024 * 1024)  // 4MB → ~125s
    ...
    if (M5.BtnA.wasPressed()) {
        if (!recording && !recorded) {
            ...
            for (int i = 0; i < samples_recorded; i++) {
                ...
                if (i % 1000 == 0) {
                    delay(10);
                    // Afficher progression toutes les 10000 samples
                    if (i % 10000 == 0) {
                        int pct = i * 100 / samples_recorded;
                        M5.Display.fillRect(0, 50, 128, 20, BLACK);
                        M5.Display.setCursor(0, 50);
                        M5.Display.printf("Envoye: %d%%\n", pct);
                        // Barre progression
                        M5.Display.fillRect(0, 70, pct*128/100, 8, CYAN);
                    }
                }
            }
            ...
            M5.Display.printf("%.1fs envoyes\n",
                (float)samples_recorded/SAMPLE_RATE);
            M5.Display.println("");
            M5.Display.setTextColor(WHITE);
            M5.Display.println("Recuperez les");
            M5.Display.println("donnees sur PC");
            M5.Display.println("avec le script");
            M5.Display.println("receive_and_plot.py");
        }
    }
